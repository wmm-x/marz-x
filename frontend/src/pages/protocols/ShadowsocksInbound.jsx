...
// Methods list including modern 2022-blake3 variants
const SS_METHODS = [
...
// Generate random password (32 bytes base64 - suitable for 2022-blake3-aes-256-gcm)
...
const getInitialState = (inbound, defaultCert) => {
  ...
  return {
    ...
    ssMethod: settings.method || '2022-blake3-aes-256-gcm',
    ssPassword: settings.password || generatePassword(), // Auto-generate if empty
    ssNetwork: settings.network || 'tcp,udp',
    ssIvCheck: settings.ivCheck !== false, // Defaults to true if undefined
...
export default function ShadowsocksInbound({ initialData, defaultCert, onClose, onSave, saving }) {
  ...
  useEffect(() => {
    if (!initialData) {
      ...
      const tag = `SS_${net}_${formData.port}`;
      setFormData(prev => ({ ...prev, tag }));
    }
  }, [formData.network, formData.port]);
  ...
  const handleSubmit = (e) => {
    ...
    const inbound = {
      ...
      protocol: "shadowsocks", 
      settings: { 
        method: formData.ssMethod,
        password: formData.ssPassword,
        network: formData.ssNetwork,
        ivCheck: formData.ssIvCheck,
        clients: [] 
      },
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        {tab === 'general' && (
          <div className="grid-2">
            ...
            <div className="card card-body" style={{ gridColumn: '1 / -1', background: 'rgba(0,0,0,0.2)', border: '1px solid var(--border-color)' }}>
              <h4 style={{ marginBottom: '16px', color: '#f59e0b' }}>Shadowsocks Settings</h4>
              
              <div className="form-group">
                <label className="form-label">Method</label>
                <select 
                  value={formData.ssMethod} 
                  onChange={e => setFormData({...formData, ssMethod: e.target.value})} 
                  className="form-input"
                >
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input 
                    ...
                    onChange={e => setFormData({...formData, ssPassword: e.target.value})} 
                    className="form-input" 
                    style={{ flex: 1 }}
                  />
                  <button 
                    type="button" 
                    onClick={() => setFormData({...formData, ssPassword: generatePassword()})} 
                    className="btn btn-primary"
                    style={{ background: '#f59e0b', borderColor: '#d97706', whiteSpace: 'nowrap' }}
                  >
                    Generate
                  </button>
                </div>
              ...
              <div className="grid-2">
                <div className="form-group">
                  ...
                  <input 
                    type="text" 
                    value={formData.ssNetwork} 
                    onChange={e => setFormData({...formData, ssNetwork: e.target.value})} 
                    className="form-input" 
                    placeholder="tcp,udp"
                  />
                </div>
                <div className="form-group" style={{ display: 'flex', alignItems: 'center', paddingTop: '28px' }}>
                  <label className="flex items-center gap-2 cursor-pointer" style={{ color: 'var(--text-secondary)' }}>
                    <input 
                      type="checkbox" 
                      checked={formData.ssIvCheck} 
                      onChange={e => setFormData({...formData, ssIvCheck: e.target.checked})} 
                    />
                    IV Check
                  </label>
                </div>
      ...
      <div className="modal-footer">
        ...
        <button type="submit" className="btn btn-primary" disabled={saving}>Save Shadowsocks</button>
      </div>
    </form>
  );
}