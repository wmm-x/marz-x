...
const getInitialState = (inbound, defaultCert) => {
  ...
  return {
    tag: inbound?.tag || '',
    port: inbound?.port || 443, // Trojan usually defaults to 443
    listen: inbound?.listen || '0.0.0.0',
    protocol: 'trojan',
    
    // Stream Defaults
    network: inbound?.streamSettings?.network || 'tcp',
    // Trojan usually implies TLS, but user might select others or none (if behind nginx)
...
export default function TrojanInbound({ initialData, defaultCert, onClose, onSave, saving }) {
  ...
  useEffect(() => {
    if (!initialData) {
      ...
      const sec = formData.security !== 'none' ? `_${formData.security.toUpperCase()}` : '';
      const tag = `TROJAN_${net}${sec}_${formData.port}`;
  ...
  const handleSubmit = (e) => {
    ...
    const inbound = {
      ...
      protocol: "trojan", // Protocol is Trojan
      settings: { 
        clients: inbound?.settings?.clients || [], // Preserve existing clients
        decryption: "none"
      },
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <div className="grid-2">
            ...
            <div className="card card-body" style={{ gridColumn: '1 / -1', background: 'rgba(0,0,0,0.2)', border: '1px solid var(--border-color)' }}>
               <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                 Note: Client passwords for Trojan are usually managed via the Users section in Marzban, not directly here.
               </p>
            </div>
          </div>
        )}
      ...
      <div className="modal-footer">
        ...
        <button type="submit" className="btn btn-primary" disabled={saving}>Save Trojan</button>
      </div>
    </form>
  );
}