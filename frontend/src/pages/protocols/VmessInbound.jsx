...
const getInitialState = (inbound, defaultCert) => {
  ...
  return {
    tag: inbound?.tag || '',
    port: inbound?.port || 10086, // Different default port for VMess commonly
...
export default function VmessInbound({ initialData, defaultCert, onClose, onSave, saving }) {
  ...
  useEffect(() => {
    if (!initialData) {
      ...
      const tag = `VMESS+${net}${sec}+${formData.port}`;
  ...
  const handleSubmit = (e) => {
    ...
    const inbound = {
      ...
      protocol: "vmess", // Protocol is VMess
      settings: { 
        clients: inbound?.settings?.clients || [] // Preserve existing clients or init empty
      },
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="modal-footer">
        ...
        <button type="submit" className="btn btn-primary" disabled={saving}>Save VMess</button>
      </div>
    </form>
  );
}